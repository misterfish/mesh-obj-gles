...
main :: IO ()
main = join $ print <$> getObjs

getObjs :: IO Sequence
getObjs = do
    materialMapMaterial <- getMaterial
    objMap <- getObjMap
    textureDir' <- T.pack <$> textureDir
    textureMap <- getTextureMap textureDir'
...
getObjMap :: IO ObjMap
getObjMap = prepare' <$> parseObj where
...
parseObj :: IO (Either String WavefrontOBJ)
parseObj = do
    file <- (<> filename) <$> framesDir
    parse file

getMaterial :: IO MaterialMapMaterial
getMaterial = do
    mtl <- join $ readFile <$> ((<> "/wolf_000001.mtl") <$> framesDir)
